...
def plot_drift_overview(shap_distances, psi_scores, ks_scores, drift_type):
    ...
    datasets = [
        (axes[0], shap_distances, config.PALETTE["shap_drift"],  "SHAP Cosine Distance", config.SHAP_DRIFT_THRESHOLD),
        (axes[1], psi_scores,     config.PALETTE["psi"],          "PSI Score",            config.PSI_ALERT_THRESHOLD),
    ...
    for ax, scores, color, label, threshold in datasets:
        ...
        ax.axhline(threshold, color=config.PALETTE["alert"], linestyle="--", linewidth=1, label="threshold")
        ax.axvline(drift_window, color="#999999", linestyle=":", linewidth=1.5, label="drift start")
        ax.fill_between(x, scores, threshold, where=(np.array(scores) > threshold),
                        alpha=0.2, color=config.PALETTE["alert"])
...
def plot_shap_vectors_over_time(shap_snapshots, feature_names, drift_type):
    ...
    ax.set_title(f"SHAP Feature Importance Over Time — {drift_type.title()} Drift", fontsize=13)
    plt.colorbar(im, ax=ax, label="Mean |SHAP|")
    ax.axvline(int(n_windows * config.DRIFT_START_FRAC), color="white", linestyle="--", linewidth=2, label="drift start")
...
def plot_latency_comparison(summary_df):
    ...
    bars = ax.bar(
        ...
        latencies, color=colors, width=0.5
    )
...
def plot_shap_reference_vs_drifted(reference_shap, drifted_shap, feature_names, drift_type):
    x = np.arange(len(feature_names))
    width = 0.35

    fig, ax = plt.subplots(figsize=(14, 5))
    ax.bar(x - width / 2, reference_shap, width, label="Reference", color=config.PALETTE["ks"])
    ax.bar(x + width / 2, drifted_shap,   width, label="Drifted",   color=config.PALETTE["alert"])
    ax.set_xticks(x)
...
def run_all_plots(shap_distances, psi_scores, ks_scores, shap_snapshots,
                  reference_shap, drifted_shap, summary_df,
                  all_summaries, feature_names, drift_type):
    ...
    plot_shap_reference_vs_drifted(reference_shap, drifted_shap, feature_names, drift_type)

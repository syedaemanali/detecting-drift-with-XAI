...
def train_all_models(
    ...
    for model_name in config.MODELS_TO_TRAIN:
        ...
        run_name = (
            model_name if not run_name_prefix else f"{run_name_prefix}_{model_name}"
        )
...
def pick_best_model(results):
    best_name = max(
        results, key=lambda name: results[name]["metrics"][config.SELECTION_METRIC]
    )
    best_score = results[best_name]["metrics"][config.SELECTION_METRIC]
    log.info(
        "Best model is %s with %s %.4f", best_name, config.SELECTION_METRIC, best_score
    )
